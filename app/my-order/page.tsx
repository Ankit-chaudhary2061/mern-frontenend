...
useEffect(() => {
  ...
  //     </div>
  //   </div>
  <div className="min-h-screen bg-gradient-to-br from-yellow-50 via-orange-50 to-green-50 py-10">
  ...
  className="w-full rounded-xl border border-orange-200 bg-orange-50 px-4 py-3 text-black focus:outline-none focus:ring-2 focus:ring-orange-400"
>
  <option value={OrderStatus.ALL}>All Orders</option>
  <option value={OrderStatus.PENDING}>Pending</option>
  ...
  value={searchTerm}
  onChange={(e) => setSearchTerm(e.target.value)}
  className="w-full rounded-xl border border-yellow-200 bg-yellow-50 px-4 py-3 text-black focus:outline-none focus:ring-2 focus:ring-yellow-400"
/>
...
<input
  ...
  value={date}
  onChange={(e) => setDate(e.target.value)}
  className="w-full rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-black focus:outline-none focus:ring-2 focus:ring-green-400"
/>
            ...
            {filteredOrders.length > 0 ? (
              filteredOrders.map((orders, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-5 border-b border-orange-100">
                    <Link href={`/my-order/${orders._id}`}>
                      <p className="font-semibold text-green-700 hover:text-orange-600 underline">
                        #{orders._id.slice(0, 12)}
                      </p>
                    </Link>
                  </td>
                  ...
                  {/* STATUS */}
                  <td className="px-6 py-5 border-b border-orange-100">
                    <span className={`inline-flex px-4 py-2 rounded-full text-sm font-bold ${getStatusColor(
                      orders.orderStatus
                    )}`}>
                      {orders.orderStatus}
                    </span>
      ...
      <div className="bg-gradient-to-r from-yellow-100 via-orange-100 to-green-100 px-6 py-4 flex flex-col md:flex-row items-center justify-between">
        ...
        <div className="flex gap-3 mt-4 md:mt-0">
          ...
          <button
            onClick={() => setPage((prev) => Math.max(prev - 1, 1))}
            disabled={page === 1}
            className="px-5 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-semibold shadow-md transition disabled:opacity-50"
          >
            Prev
          </button>
          ...
          <button
            onClick={() => setPage((prev) => prev + 1)}
            disabled={filteredOrders.length < 10}
            className="px-5 py-2 rounded-xl bg-green-600 hover:bg-green-700 text-white font-semibold shadow-md transition disabled:opacity-50"
          >
            Next
          </button>
